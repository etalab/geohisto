...
from geohisto.actions import (
    ...
    do_obsoletes, do_all_actions, do_change_county
)
...
def test_rename(towns, history_list):  # NOQA: fixtures.
    ...
    assert neuville_s.id == '102631942-01-012008-10-06'
    assert neuville_s.nccenr == 'Neuville-sur-Vannes'
    assert neuville_s.start_date == datetime.date(1942, 1, 1)
    assert neuville_s.end_date == datetime.date(2008, 10, 6)
    assert neuville_s.modification == RENAME_SIMPLE
    assert neuville_s.successors == neuville.id
    assert neuville.id == '102632008-10-069999-01-01'
    assert neuville.nccenr == 'Neuville-sur-Vanne'
    assert neuville.start_date == datetime.date(2008, 10, 6)
    assert neuville.end_date == datetime.date(9999, 1, 1)
...
def test_fusion(towns, history_list):  # NOQA: fixtures.
    ...
    assert braguelogne.id == '100581942-01-011973-05-01'
    ...
    assert braguelogne_beauvoir.id == '100581973-05-019999-01-01'
    assert braguelogne_beauvoir.nccenr == 'Bragelogne-Beauvoir'
    assert beauvoir_sur_sarce.id == '100361942-01-011973-05-01'
...
def test_fusion_then_split(towns, history_list):  # NOQA: fixtures.
    ...
    assert framboisiere.id == '281591942-01-011972-12-22'
    ...
    assert framboisiere_saucelle.id == '281591972-12-229999-01-01'
    assert framboisiere_saucelle.nccenr == 'Framboisière-la-Saucelle'
    assert saucelle.id == '283681942-01-011972-12-22'
...
def test_fusion_then_reinstatement_then_split(towns, history_list):  # NOQA: fixtures.
    ...
    assert varenne.id == '525041942-01-011972-08-01'
    assert terre_natale.id == '525041972-08-012012-01-01'
    assert varenne2.id == '525042012-01-019999-01-01'
    assert champigny.id == '521031942-01-011972-08-01'
    assert champigny2.id == '521031986-01-019999-01-01'
    assert chezeau.id == '521241942-01-011972-08-01'
    assert chezeau2.id == '521242012-01-019999-01-01'
...
def test_fusions_to_new_delegated(towns, history_list):  # NOQA: fixtures.
    ...
    assert illtal.id == '682402016-01-019999-01-01'
    assert obedorf.id == '682401942-01-012016-01-01'
    assert grentzingen.id == '681081942-01-012016-01-01'
    assert henflingen.id == '681331942-01-012016-01-01'
...
def test_fusions_to_new_not_delegated(towns, history_list):  # NOQA: fixtures.
    ...
    assert fragnes.id == '712041942-01-012016-01-01'
    assert loyere.id == '712651942-01-012016-01-01'
    assert fragnes_loyere.id == '712042016-01-019999-01-01'
...
def test_deletions(towns, history_list):  # NOQA: fixtures.
    ...
    assert creusy.id == '451171942-01-011965-01-01'
...
def test_obsoletes(towns, history_list):  # NOQA: fixtures.
    ...
    assert hauteville_lompnes.id == '014591942-01-011942-08-01'
    assert hauteville_lompnes.successors == '011851942-01-019999-01-01'
...
def test_change_county(towns, history_list):  # NOQA: fixtures.
    ...
    assert old_afa.id == '200011942-01-011976-01-01'
    assert new_afa.id == '2A0011976-01-019999-01-01'
    assert old_afa.successors == '2A0011976-01-019999-01-01'
